...
To use the "Time Events" feature in Motion Canvas, it's important to register your event with the scene using the following code:
```ts
useScene().timeEvents.register('event');
```

This line should be added before calling the waitUntil function.
